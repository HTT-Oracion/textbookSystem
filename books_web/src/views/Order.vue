<template>
  <div class="order-page">
    <el-card>
      ...
      <el-table
        :data="orderList.slice((pageNum - 1) * pageSize, pageNum * pageSize)"
        border
        stripe
        v-loading="loading"
      >
        <el-table-column label="订单编号">
          <template #default="scope">
            <span class="order-id" @click="showOrder(scope.row.id)">
              {{ scope.row.id }}</span
            >
          </template>
        </el-table-column>
        ...
        <el-table-column
          prop="charge.username"
          label="订购人姓名"
        ></el-table-column>
        <el-table-column
          prop="approval.username"
          label="审批人姓名"
        ></el-table-column>
        ...
        <el-table-column label="订购状态">
          <template #default="scope">
            <el-tag v-if="scope.row.charge_status === 0" type="danger"
              >未订购</el-tag
            >
        ...
        <el-table-column label="审核状态">
          <template #default="scope">
            <el-tag v-if="scope.row.approval_status === 1" type="primary"
              >已审批</el-tag
            >
        ...
        <el-table-column label="是否入库">
          <template #default="scope">
            <el-tag v-if="scope.row.charge_status === 2" type="primary"
              >已入库</el-tag
            >
            <el-tag v-else type="warning">未入库</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="入库操作" width="250">
          <template #default="scope">
            <el-button
              type="success"
              size="mini"
              :disabled="scope.row.approval_status === 1"
              @click="approval(user.id, scope.row)"
              >审批</el-button
            >
            <el-button
              type="warning"
