...
const { Charge, Approval, Class, Department, Major, Book } = require('../models')
module.exports = (app, User, Sequelize) => {
  ...
  router.get('/charge/list', async (req, res) => {
    ...
    if (resultList.length === 0) return res.send({ status: 422, msg: '没有任数据!' })
    return res.send({ status: 200, msg: `获取列表成功`, result: resultList, total })
  })
  ...
  router.put('/:id', async (req, res) => {
    ...
    try {
      ...
      if (level === 0) {
        await Charge.update({ phone, username }, { where: { id: req.params.id } })
        await Approval.update({ phone, username }, { where: { id: req.params.id } })
      } else if (level === 2) {
        await Charge.update({ phone, username }, { where: { id: req.params.id } })
      } else if (level === 3) {
        await Approval.update({ phone, username }, { where: { id: req.params.id } })
